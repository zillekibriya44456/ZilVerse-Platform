...
router.post('/sandbox/run', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        try {
            vm_1.default.runInContext(code, context, { timeout: 1500 });
        }
        catch (compileErr) {
            return res.json({
                success: false,
        ...
        if (testCases && Array.isArray(testCases)) {
            for (let i = 0; i < testCases.length; i++) {
                ...
                try {
                    ...
                    const resultVal = vm_1.default.runInContext(`${fnName}(${JSON.stringify(tc.input)})`, context, { timeout: 500 });
