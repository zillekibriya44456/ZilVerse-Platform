...
const stripe = new stripe_1.default(process.env.STRIPE_SECRET_KEY || 'dummy_stripe_secret_key', {
...
function emitWalletUpdate(req, userId) {
...
router.post('/escrow/create', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (isNaN(escAmount) || escAmount <= 0)
            return res.status(400).json({ error: 'Invalid amount' });
        if (clientId === freelancerId) {
            return res.status(400).json({ error: 'You cannot initiate an escrow transaction with yourself.' });
        }
        const freelancerUser = await prisma.user.findUnique({ where: { id: freelancerId } });
        if (!freelancerUser)
            return res.status(400).json({ error: 'Freelancer account not found.' });
...
router.post('/escrow/release', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.post('/escrow/dispute', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.get('/escrows', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.get('/withdrawals', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
...
router.get('/admin/summary', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.post('/admin/withdrawals/:id/approve', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.post('/admin/withdrawals/:id/reject', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id: idParam } = req.params;
        const id = idParam;
        const reqObj = await prisma.withdrawalRequest.findUnique({ where: { id } });
...
router.post('/admin/disputes/:id/resolve', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id: idParam } = req.params;
        const id = idParam;
        ...
        const dispute = await prisma.dispute.findUnique({ where: { id }, include: { escrow: true } });
        if (!dispute)
